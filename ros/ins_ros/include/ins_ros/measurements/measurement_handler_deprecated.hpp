...
class MeasurementHandler {
 public:
  ...
  /**
   * @brief Closest N measurements to t_query within tolerance in chronological order (oldest first).
   *
   * Only looks at measurements with stamp <= t_query + future_tolerance
   * (to allow for small timestamp jitter). Consumes everything up to and
   * including the returned N measurements so it cannot be reused.
   * Returns empty vector if nothing is within tolerance.
   */
  std::vector<StampedGps> takeSyncNGps(double t_query, std::size_t n,
                                      double tolerance, double future_tolerance = 0.02);
  std::vector<StampedOdom> takeSyncNOdom(double t_query, std::size_t n,
                                    double tolerance, double future_tolerance = 0.02);       
  std::vector<StampedWheel> takeSyncNWheel(double t_query, std::size_t n,
                                    double tolerance, double future_tolerance = 0.02);
  std::vector<StampedMag> takeSyncNMag(double t_query, std::size_t n,
                                    double tolerance, double future_tolerance = 0.02);
  std::vector<StampedBaro> takeSyncNBaro(double t_query, std::size_t n,
                                    double tolerance, double future_tolerance = 0.02);
  std::vector<StampedYaw> takeSyncNYaw(double t_query, std::size_t n,
                                    double tolerance, double future_tolerance = 0.02);       
  ...
  std::size_t stateQueued() const;
  std::size_t imuHistoryQueued() const;
...
//  private:
 public:
  ...
  template <typename T, typename StampFn>
  static std::vector<T> takeClosestNTo(std::deque<T>& buffer, double t_query,
                                        std::size_t n, double tolerance,
