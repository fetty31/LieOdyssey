...
template <typename T, typename StampFn>
std::vector<T> MeasurementHandler::takeClosestNTo(std::deque<T>& buffer, double t_query,
                                                  std::size_t n,
                                                  double tolerance,
                                                  double future_tolerance,
                                                  StampFn stamp_of) 
{
  ...
  if (buffer.size() < n) // avoid consuming before buffer is filled
    return out;

  const double t_min = t_query - tolerance;
  const double t_max = t_query + future_tolerance;

  // Remove samples that are definitely too old.
  while (!buffer.empty() && stamp_of(buffer.front()) < t_min) {
    buffer.pop_front();
  }
  ...
  // Find all samples inside the valid time window.
  std::vector<std::pair<double, std::size_t>> candidates;
  candidates.reserve(buffer.size());

  for (std::size_t i = 0; i < buffer.size(); ++i) {
    const double stamp = stamp_of(buffer[i]);

    if (stamp > t_max)
      break;

    candidates.emplace_back(
        std::abs(stamp - t_query), i);
  }

  if (candidates.empty())
    return out;

  // Keep only the n closest samples.
  const std::size_t count = std::min(n, candidates.size());
  ...
  for (std::size_t i = 0; i < count; ++i)
    selected_indices.push_back(candidates[i].second);
  ...
  //
  // Erase backwards so that indices remain valid.
  std::sort(
      selected_indices.rbegin(),
      selected_indices.rend());

  for (const auto idx : selected_indices)
    buffer.erase(buffer.begin() + static_cast<std::ptrdiff_t>(idx));
...
std::vector<StampedGps> MeasurementHandler::takeSyncNGps(double t_query, std::size_t n,
                                                              double tolerance, double future_tolerance) 
{
  std::lock_guard<std::mutex> lock(mutex_);

  return takeClosestNTo(gps_buffer_, t_query, n,
                        tolerance, future_tolerance,
                        [](const StampedGps& e) { return e.stamp; });
}

std::vector<StampedOdom> MeasurementHandler::takeSyncNOdom(double t_query, std::size_t n,
                                                              double tolerance, double future_tolerance) 
{
  std::lock_guard<std::mutex> lock(mutex_);

  return takeClosestNTo(odom_buffer_, t_query, n,
                        tolerance, future_tolerance,
                        [](const StampedOdom& e) { return e.stamp; });
}

std::vector<StampedWheel> MeasurementHandler::takeSyncNWheel(double t_query, std::size_t n,
                                                              double tolerance, double future_tolerance) 
{
  std::lock_guard<std::mutex> lock(mutex_);

  return takeClosestNTo(wheel_buffer_, t_query, n,
                        tolerance, future_tolerance,
                        [](const StampedWheel& e) { return e.stamp; });
}

std::vector<StampedMag> MeasurementHandler::takeSyncNMag(double t_query, std::size_t n,
                                                              double tolerance, double future_tolerance) 
{
  std::lock_guard<std::mutex> lock(mutex_);

  return takeClosestNTo(mag_buffer_, t_query, n,
                        tolerance, future_tolerance,
                        [](const StampedMag& e) { return e.stamp; });
}

std::vector<StampedBaro> MeasurementHandler::takeSyncNBaro(double t_query, std::size_t n,
                                                              double tolerance, double future_tolerance) 
{
  std::lock_guard<std::mutex> lock(mutex_);

  return takeClosestNTo(baro_buffer_, t_query, n,
                        tolerance, future_tolerance,
                        [](const StampedBaro& e) { return e.stamp; });
}

std::vector<StampedYaw> MeasurementHandler::takeSyncNYaw(double t_query, std::size_t n,
                                                              double tolerance, double future_tolerance) 
{
  std::lock_guard<std::mutex> lock(mutex_);

  return takeClosestNTo(yaw_buffer_, t_query, n,
                        tolerance, future_tolerance,
                        [](const StampedYaw& e) { return e.stamp; });
}
...
std::size_t MeasurementHandler::stateQueued() const {
  std::lock_guard<std::mutex> lock(mutex_);
  return state_history_.size();
}

std::size_t MeasurementHandler::imuHistoryQueued() const {
  std::lock_guard<std::mutex> lock(mutex_);
