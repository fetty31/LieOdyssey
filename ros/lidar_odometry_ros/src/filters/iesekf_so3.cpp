...
Group lidar_odometry_ros::iESEKF::get_filled_state(const V3& p, const Quat& q, const V3& v, const V3& b_w, const V3& b_a, const V3& gravity)
{
	using NativeBundle = manif::Bundle<iESEKF::Scalar, 
                                            ...
                                            manif::R3      // gravity 
                                            >;
	using SO3 = manif::SO3<iESEKF::Scalar>;
	using R3    = manif::R3<iESEKF::Scalar>;

	auto X0 = NativeBundle( R3(p),					// position              0
							SO3(q),                 // orientation           3
							R3(v),                  // velocity              6      
							R3(b_w), 				// b_w                   9
							R3(b_a), 				// b_a                   12
...
void lidar_odometry_ros::iESEKF::group_to_state(const Group& g, V3& p, Quat& q, V3& v, V3& b_w, V3& b_a, V3& gravity)
{
	...
	p = X.subgroup<0>().coeffs();
	q = X.subgroup<1>().quat();
	v = X.subgroup<2>().coeffs();

	// biases
	b_w = X.subgroup<3>().coeffs();
	b_a = X.subgroup<4>().coeffs();

	// gravity
	gravity = X.subgroup<5>().coeffs();
}
...
typename Filter::Tangent lidar_odometry_ros::iESEKF::f_state(const Group& g, const lie_odyssey::IMUmeas& imu) 
{
	...
	auto grav = g.impl().subgroup<5>().coeffs(); 			    // gravity vector estimate
	auto R = g.impl().subgroup<1>().quat().toRotationMatrix();	// orientation estimate
	auto v0 = g.impl().subgroup<2>().coeffs();					// velocity estimate
...
void lidar_odometry_ros::iESEKF::fill_H_point_to_plane(const Group& group, 
														...
														HMat& H)
{
	// To-Do: concatenate jacobians of point-to-plane residual w.r.t position and orientation (R3 + SO(3) part of the state)
}

void lidar_odometry_ros::iESEKF::degeneracy_callback(const Filter& /*kf*/, Tangent& dx, const MatDoF& HRH)
{
	/* 
		To-Do: Here we handle degeneracy in SO3 group (second subgroup of our Bundle state)
	*/
}