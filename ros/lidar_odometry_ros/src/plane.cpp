...
// class lidar_odometry_ros::Plane
    // public
        ...
        lidar_odometry_ros::Plane::Plane(const MapPoints& pts, const std::vector<float>& dts, 
                                Config::Mapping* config_ptr) : is_plane(false),
                                                                 cfg_ptr(config_ptr) { 
        ...
        Eigen::Vector4f lidar_odometry_ros::Plane::get_normal(){
            return this->n_ABCD;
        }

        bool lidar_odometry_ros::Plane::good_fit(){
            return this->is_plane;
        }

        bool lidar_odometry_ros::Plane::enough_points(const MapPoints& pts){
            return this->is_plane = pts.size() >= cfg_ptr->NUM_MATCH_POINTS;
        }

        bool lidar_odometry_ros::Plane::close_enough(const std::vector<float>& dts){
            if(dts.size() < 1) return this->is_plane = false;
            return this->is_plane = dts.back() < cfg_ptr->MAX_DIST_PLANE;
        }

        float lidar_odometry_ros::Plane::dist2plane(const Eigen::Vector3f& p) const {
            return n_ABCD(0) * p(0) + n_ABCD(1) * p(1) + n_ABCD(2) * p(2) + n_ABCD(3);
        }

        float lidar_odometry_ros::Plane::dist2plane(const PointType& p) const {
            return n_ABCD(0) * p.x + n_ABCD(1) * p.y + n_ABCD(2) * p.z + n_ABCD(3);
        }

        bool lidar_odometry_ros::Plane::on_plane(const Eigen::Vector3f& p) {
            if(not this->is_plane) return false;
            return std::fabs(this->dist2plane(p)) < cfg_ptr->PLANE_THRESHOLD;
        }

        bool lidar_odometry_ros::Plane::on_plane(const PointType& p) {
            if(not this->is_plane) return false;
            return std::fabs(this->dist2plane(p)) < cfg_ptr->PLANE_THRESHOLD;
        }
    ...
    // private
        ...
        void lidar_odometry_ros::Plane::fit_plane(const MapPoints& pts){
            // Estimate plane
        ...
        Eigen::Vector4f lidar_odometry_ros::Plane::estimate_plane(const MapPoints& pts){
        ...
        bool lidar_odometry_ros::Plane::plane_eval(const Eigen::Vector4f& n, const MapPoints& pts, const float& thres){
        ...
        Eigen::Vector3f lidar_odometry_ros::Plane::get_centroid(const MapPoints& pts){
            int N = pts.size();
