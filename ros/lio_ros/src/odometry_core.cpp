...
lio_ros::OdometryCore::OdometryCore() 
{
    ...
    this->raw_world_scan_ = pcl::PointCloud<LioPointType>::Ptr (lio_ros::make_shared<pcl::PointCloud<LioPointType>>());
    this->pc2match_       = pcl::PointCloud<LioPointType>::Ptr (lio_ros::make_shared<pcl::PointCloud<LioPointType>>());
}

void lio_ros::OdometryCore::initialize(const lio_ros::Config& config)
{
    this->config_ = config;
    ...
    map_ = std::make_unique<Map>(max_num_threads_,
                                config.n_points_match,
                                config.plane_threshold,
                                config.max_num_matches,
                                config.octree_bucket_size,
                                config.octree_extent
                                );
...
{ // Preprocess scan (NaN removal, cropping, downsampling, motion compensation)
    ...
    pcl::PointCloud<LioPointType>::Ptr input_pc (lio_ros::make_shared<pcl::PointCloud<LioPointType>>());
    ...
    if (this->config_.voxel_active) { 
        pcl::PointCloud<LioPointType>::Ptr current_scan_
            (lio_ros::make_shared<pcl::PointCloud<LioPointType>>(*deskewed_Xt2_pc_));
    ...
    if(this->pc2match_->points.size() > 1){
        ...
        pcl::PointCloud<LioPointType>::Ptr mapped_scan (lio_ros::make_shared<pcl::PointCloud<LioPointType>>());
...
void lio_ros::OdometryCore::pointToPlaneResidual(const iESEKF::Group& X_now, iESEKF::Measurement& z, iESEKF::HMat& H)
{
    lio_ros::State st;
    ...
    for (std::size_t i = 0; i < N; ++i) {
        ...
        n     = match.plane.coeffs.head(3).cast<Scalar>();
        n.normalize();
        ...
        z(i) = -Scalar(match.plane.distance(match.point));
    }
}

lio_ros::SensorType lio_ros::OdometryCore::get_sensor_type() const
{
    return config_.sensor_type;
}

// Get covariance (6x6 pose + 6x6 twist flattened row-major)
std::vector<double> lio_ros::OdometryCore::getPoseCovariance() const
{
    return iESEKF::get_pose_covariance(this->filter_->getCovariance());
}

std::vector<double> lio_ros::OdometryCore::getTwistCovariance() const
{
...
pcl::PointCloud<LioPointType>::Ptr lio_ros::OdometryCore::deskewScan(pcl::PointCloud<LioPointType>::Ptr& pc, double& start_time)
{
    ...
    if (this->config_.sensor_type == lio_ros::SensorType::OUSTER) {
    ...
    } else if (this->config_.sensor_type == lio_ros::SensorType::VELODYNE) {
    ...
    } else if (this->config_.sensor_type == lio_ros::SensorType::HESAI) {
    ...
    } else if (this->config_.sensor_type == lio_ros::SensorType::LIVOX) {
        ...
        std::cout << "-------------------------------------------------------------------\n";
        std::cout << "lio_ros::FATAL ERROR: LiDAR sensor type unknown or not specified!\n";
        std::cout << "-------------------------------------------------------------------\n";
        return lio_ros::make_shared<pcl::PointCloud<LioPointType>>();
    }

    // copy points into deskewed_scan in order of timestamp
    pcl::PointCloud<LioPointType>::Ptr deskewed_scan (lio_ros::make_shared<pcl::PointCloud<LioPointType>>());
    ...
    if(deskewed_scan->points.size() < 1){
        std::cout << "lio_ros::ERROR: failed to sort input pointcloud!\n";
        return lio_ros::make_shared<pcl::PointCloud<LioPointType>>();
    }
    ...
    for (std::size_t k = 0; k < deskewed_scan->points.size(); k++) {
        ...
        int i_f = lio_ros::binary_search_tailored(frames, extract_point_time(deskewed_scan->points[k])+offset);

        lio_ros::State X0 = frames[i_f];
...
std::vector<lio_ros::State> lio_ros::OdometryCore::integrateImu(double start_time, double end_time)
{
    ...
    if (not this->propagatedFromTimeRange(start_time, end_time, begin_prop_it, end_prop_it)) {
        ...
        std::cout << "lio_ros::propagatedFromTimeRange(): not enough propagated states!\n";
        return states;
    }
...
bool lio_ros::OdometryCore::propagatedFromTimeRange(double start_time, double end_time,
    ...
    end_prop_it = boost::circular_buffer<lio_ros::State>::reverse_iterator(last_prop_it);
    begin_prop_it = boost::circular_buffer<lio_ros::State>::reverse_iterator(prop_it);
