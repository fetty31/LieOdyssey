...
// ---------------------- Test Fixture ----------------------
class Pose3DMeasurementTest : public ::testing::Test {
protected:
    Filter filter;
    IMUmeas imu;
    double dt{0.01};

    Vec3 gt_pos{1.0, 2.0, 3.0};
    double yaw_gt{0.3};
...
TEST_F(Pose3DMeasurementTest, GroundTruthZeroResidual) {
    Group meas_state = make_group(gt_pos, gt_quat, gt_vel, gt_bg, gt_ba, gt_g, gt_time);

    Eigen::Matrix<Scalar,9,9> R = Eigen::Matrix<Scalar,9,9>::Zero();
    R.diagonal() = Eigen::Matrix<Scalar, 9, 1>::Constant(Scalar(0.01));
    auto R_inv = R.inverse();

    auto state_before = filter.getState();
    filter.update<Group, Measurement, HMat>(meas_state, R, R_inv, ins_ros::iESEKF::odom::H_fun);
...
TEST_F(Pose3DMeasurementTest, GroundTruthWithSmallNoise) {
    Vec3 noise_pos(0.01, -0.005, 0.008);
    double noise_yaw = 0.01;
    Quat noisy_quat{Eigen::AngleAxisd(noise_yaw, Vec3::UnitZ())};

    Group meas_state = make_group(gt_pos + noise_pos, noisy_quat, gt_vel, gt_bg, gt_ba, gt_g, gt_time);
...
TEST_F(Pose3DMeasurementTest, GroundTruthWithVelocity) {
    Vec3 meas_vel(0.15, 0.02, 0.0);
    Group meas_state = make_group(gt_pos, gt_quat, meas_vel, gt_bg, gt_ba, gt_g, gt_time);
...
TEST_F(Pose3DMeasurementTest, CovarianceReduction) {
    Group meas_state = make_group(gt_pos, gt_quat, gt_vel, gt_bg, gt_ba, gt_g, gt_time);
...
TEST_F(Pose3DMeasurementTest, HighNoisePoseDoesNotMoveStateMuch) {
    Group meas_state = make_group(gt_pos + Vec3(10.0, 10.0, 10.0),
                                          Quat(Eigen::AngleAxisd(1.0, Vec3::UnitZ())),
                                          gt_vel, gt_bg, gt_ba, gt_g, gt_time);

    Eigen::Matrix<Scalar,9,9> R = Eigen::Matrix<Scalar,9,9>::Zero();
    R.diagonal() = Eigen::Matrix<Scalar, 9, 1>::Constant(Scalar(100.0));
    auto R_inv = R.inverse();
    ...
    auto dx = state_after.minus(state_before);
    EXPECT_LT(dx.coeffs().norm(), 1.0);
}

// ---------------------- Identity State Pose Test ----------------------

TEST(Pose3DTest, IdentityStateGroundTruth) {
    using SGal3 = manif::SGal3<Scalar>;
    using R3 = manif::R3<Scalar>;
    using NativeBundle = manif::Bundle<Scalar, manif::SGal3, manif::R3, manif::R3, manif::R3>;
...
// ---------------------- Pose Prediction Test ----------------------

TEST_F(Pose3DMeasurementTest, PredictMaintainsStateStructure) {
